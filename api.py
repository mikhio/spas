...
def addCashback(idp):
    ...
    if date < partners[idp]['datestop']:
        partners[idp]['spent_budget'] += int(request.json['cashback'])
        partners[idp]['data'].loc[len(partners[idp]['data'])] = [idp, int(request.json['cashback']), date]
        # partners[idp]['is_stopped'] = off(idp)
...
def off(idp):
    ...
    return pred >= need * 1.1 or need < 0  #если предположительная сумма превосходит имеющуюся, то останавливаем  (берём с запасом)
